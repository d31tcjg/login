<template>
  ...
  >
    <form @submit.prevent class="flex flex-col w-full p-12 shadow-lg">
      ...
      <button
        @click="google"
        class="btn btn-primary-content mt-8"
        v-if="form == 'signup'"
      >
        <img class="w-8" src="../assets/google.webp" alt="Google Logo" />Sign Up
        With Google
      </button>
      <button @click="google" class="btn btn-primary-content mt-8" v-else>
        <img class="w-8" src="../assets/google.webp" alt="Google Logo" />Sign in
        With Google
      </button>
    </form>
  </section>
